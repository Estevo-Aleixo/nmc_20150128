...
$error_no_asr = array("error" => True, "value" => "There's no craptions available for this video");
...
function format_string($string, $data) {
...
function var_check(&$var, $return_value = False) {
    if (isset($var)) {
        if(!empty($var)) {
            if($return_value) {
                return $var;

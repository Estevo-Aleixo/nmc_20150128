...
class Youtube {
    ...
    protected $url_youtube_retry_limit = 10;
    protected $itags = array(18, 5, 36, 17); // 43 != IE
...
/*    
        ...
        YE OLDE GET_BEST_STREAM FUNCTIONALITY
        ...
        foreach($this->itags as $itag) {
            foreach($this->streams as $stream) {
                if (!strpos($stream['type'],'webm')) {
                    continue ;
                }
                if ($stream['type']== 'video/mp4; codecs="avc1.42001E, mp4a.40.2"' || $stream['type']=='video/x-flv'
                    || $stream['type']=='video/3gpp; codecs="mp4v.20.3, mp4a.40.2"')
                {
                    continue;
                }
                return $stream ;
...
*/
        ...
        foreach($this->itags as $itag) {
            foreach($this->streams as $stream) {
                ...
                if (preg_match('/Opera/i',$u_agent) && !strpos($stream['type'],'webm')) {
                    continue ;
                }
                return $stream ;
                if($itag == $stream['itag']) {
    ...
    function retrieve_asr_caption() { // Retrieve asr caption, need tts_url and langcode
        ...
        while(in_array(curl_getinfo($curl)['http_code'], array(0, '301', '404')) && $retry < $this->url_youtube_retry_limit) { // Keep trying until get or reach limit
            $curl = curl_prepare($url);
            $data = curl_exec($curl);        
            $retry++;
        }
